...
export const TOOLTIPS: Record<string, string> = {
  ...
  '業務に伴う責任の程度': 'このスタッフが他のスタッフへの指示・管理などリーダー的な役割を担うかどうかです。派遣先との個別契約の内容を確認のうえ、選択してください。',
  // 2026-07-29デモ指摘①：給与項目（基本給・各種手当）へのヘルプ説明追加
  '基本給': '毎月固定で支払われる給与の基本となる金額です。各種手当を含まない金額を入力してください。',
  '定額残業手当': 'あらかじめ一定時間分の残業代を固定額で支払う手当です。設定時間を超えた分は別途支払います。',
  '職能給': 'SP手当やスタッフの技能・資格・経験等に応じて支払う手当です。',
  '役職手当': '役職（主任・GL・課長など）に対して支払う手当です。役職がない場合は入力不要です。',
  '営業手当': '営業活動に従事することに対して支払う手当です。',
  '住宅手当': '家賃補助など住居費用の一部を補助する手当です。',
  // 2026-07-29：派遣期間・雇用期間の入力制約をヘルプアイコンで案内（伊藤さん指摘）
  '組織単位': '抵触日を管理する単位となる、派遣先の部署名（課・グループなど）を入力してください。',
  '契約条件適用開始日': '無期契約・正社員の方に適用する契約条件の開始日です。派遣期間の開始日と同じか、それより後の日付にしてください。',
  '雇用期間（兼用）': '雇用期間の開始日は派遣期間の開始日と同じか、それより後の日付に、終了日は派遣期間の終了日と同じか、それより後の日付にしてください。',
  '試用期間': '「有」を選んだ場合、開始日は雇用期間の開始日と同じ日付に、終了日は雇用期間の終了日以前かつ開始日より後の日付にしてください。',
}
...
// 試用期間の重大警告文（2026-08-06：StepPeriod.tsx＝入力時とStepFinalCheck.tsx＝最終確認時に
// 同一の文言が別々にコピーされていたため共通化。片方だけ文言を直して他方が古いまま、という事故を防ぐ）。
export const getTrialOver6WarningMessage = (months: number, days: number) =>
  `就業規則第13条では試用期間は原則6ヶ月以内と定められています。\n入力された試用期間（${months}ヶ月${days > 0 ? days + '日' : ''}）は6ヶ月を超えています。\n延長が必要な場合は就業規則第13条第2項に基づき、本人への2週間前通知が必要です。\n本当にこのまま申請してよろしいですか？`

export const NO_TRIAL_PERIOD_WARNING_MESSAGE =
  '正社員の雇用では原則として試用期間（6ヶ月）が設けられます（就業規則第13条）。\n試用期間「無し」で申請する場合は、会社が適当と認めた特別なケースに限られます。\n本当にこのまま申請してよろしいですか？'
