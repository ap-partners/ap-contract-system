...
- 差し戻し案件：`isRejected=true` 時に差し戻しバナー表示（※現状ハードコード。実データ連携は未実装 → 9章PENDING参照）
- 警告確認：試用期間超・給与100万円超等の警告はチェックボックス必須（未チェックで申請不可）
- 申請ボタン：確認モーダル（スタッフ名・帳票種類・雇用区分）→ `contracts` テーブルにinsert → 完了画面
...
### 9-1. 実装PENDING（優先順）
1. **【フェーズ2】差し戻し再申請フロー**
   - `/apply` を再利用（`/apply?edit=契約ID` 形式）
   - ⚠️ **現状の `isRejected` 関連state（`rejectionReason`・`rejectedAt`・`rejectedBy`）は全てハードコードされたダミー**。実データ連携も編集モードの仕組みも存在しない。再申請フロー実装時はここから設計が必要。
   - URLパラメータ受け取り → `contracts` テーブルから既存データ取得 → 各stateに復元 → 申請時はinsertではなくupdateに分岐
2. **【フェーズ2以降】SSC一括承認の完全実装**
   - 警告なし案件のみチェックボックス選択 → 一括承認（UIは実装済み）
   - 承認後の後続処理（担当営業への通知メール）は未実装
3. **4システム動作確認テスト**：e-staffing済。HRstation・winworks・Staffia 未テスト。
4. **派遣元情報（STEP4）のCSV優先表示**
5. **CSV反映項目を担当営業が修正した場合の特別対応**
6. **STEP1 UIに「アルバイト」選択肢追加**
7. **管理部ダッシュボード**
8. **自動チェック機能**：金額異常値、最低賃金、就業規則整合 → `contracts.auto_check_results`へ格納
9. **承認後の自動送信処理（残課題）**：担当営業ダッシュボードに「説明完了」ボタンを実装済み（締結パターンが対面／印刷の場合、押下で`status`を署名待ちに進め`sign_requested_at`を記録）。ただし①締結パターン「指定しない」時にSSC承認と同時に自動でスタッフへ送る仕組み、②実際のメール送信・署名依頼URL発行、はまだ未実装（`/sign/[id]`実装＝フェーズ5と合わせて着手予定）。
