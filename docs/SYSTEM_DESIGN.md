...
| `created_by_dept_no` | integer | YES | null | 申請者の部門NO（申請時スナップショット。部門フィルタ用）※フェーズ2で追加予定 |
...
### 9-1. 実装PENDING（優先順）
1. **【フェーズ2・最優先】担当営業ダッシュボード**
   - 閲覧範囲：自部門の全申請（業務継続性のため。同部門の担当営業が休んでも他メンバーが対応できる）
   - 部門判定：ログインユーザーの `email` で `staff` テーブルを検索 → `dept_no` 取得 → `contracts.created_by_dept_no` で絞り込み
   - **`contracts.created_by_dept_no`（integer）を新規追加予定**：申請時に申請者の `dept_no` をスナップショット保存。異動後も申請時点の部門で管理できる。
   - ⚠️ **前提条件**：`staff.email` が StaffExpress エクスポートの実メールアドレスに解除されるまで、この機能は正しく動作確認できない（現在は `ito@appart.co.jp` に全員固定中）
2. **【フェーズ2】差し戻し再申請フロー**
...
3. **【フェーズ2以降】SSC一括承認の完全実装**
   - 警告なし案件のみチェックボックス選択 → 一括承認（UIは実装済み）
   - 承認後の後続処理（担当営業への通知・締結パターン別URL送信）は未実装
4. **4システム動作確認テスト**：e-staffing済。HRstation・winworks・Staffia 未テスト。
5. **派遣元情報（STEP4）のCSV優先表示**
6. **CSV反映項目を担当営業が修正した場合の特別対応**
7. **STEP1 UIに「アルバイト」選択肢追加**
8. **管理部ダッシュボード**
9. **自動チェック機能**：金額異常値、最低賃金、就業規則整合 → `contracts.auto_check_results`へ格納
10. **承認後の後続処理**：担当営業へ承認通知メール、締結パターン別URL自動送信（指定しない→即時、B/C→担当営業の「説明完了」ボタン押下後）、スタッフへの署名依頼URL生成
