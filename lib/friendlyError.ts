...
const PG_ERROR_MESSAGES: Record<string, string> = {
  ...
  // B-15対応（2026-08-12）：同一バッチ内の重複キーでON CONFLICT DO UPDATEが同じ行を2回更新
  // しようとした場合のエラー。デデュープ処理を入れたため通常は発生しないはずだが、
  // 万が一発生した場合に原因追跡できるよう防御的にマッピングしておく。
  '21000': 'データの重複により保存できませんでした。ファイル内に同じ番号のデータが複数含まれていないかご確認ください。',
}
