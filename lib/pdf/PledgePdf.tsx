...
// 2026-07-23追記（伊藤さんレビュー対応）：①就業日程は常に10行の枠を用意（空欄も明示）し
// 下部の余白を解消、②業務内容欄の縦幅を拡大、③賃金支払方法・交通費は雇用契約書と同じ固定文言
// （WAGE_PAYMENT_TEXT・getTransportText/getTransportSecondaryNote）をこのファイルで直接参照する
// 方式に変更（保存データに依存せず常に契約書と同一内容を保証するため）、④各行の縦の余白を拡大。
import { Document, Page, Text, View, StyleSheet } from '@react-pdf/renderer'
import {
  PLEDGE_DOCUMENT_TITLE, PLEDGE_INTRO_TEXT, PLEDGE_CLOSING_TEXT, formatYen,
  WAGE_PAYMENT_TEXT, getTransportText, getTransportSecondaryNote,
} from './documentText'
import {
...
export interface PledgePdfProps {
  ...
  showSeal: boolean
  // 従業員の丸印鑑（PNGのdata URL）。署名完了時にのみ渡される。
  signatureImageDataUrl?: string
}

// v5仕様：「就業日程」表は行ごとに別テーブルにすると罫線が二重になるため、1つの表にまとめる。
// 外枠のみ太枠（外側のLabeledRow.valueCellが担当）、内部は下端・右端の線のみを引く方式。
// 2026-07-23追記：単日は最大10件までのため、実データが何行であっても常に10行分の枠を用意し、
// 余った行は空欄で表示する（伊藤さん指摘：枠を埋めることで下部の余白を解消する）。
const scheduleStyles = StyleSheet.create({
  ...
  colDate: { width: '32%', padding: '6 5', borderRightWidth: THIN, borderColor: BORDER, justifyContent: 'center' },
  colTime: { width: '30%', padding: '6 5', borderRightWidth: THIN, borderColor: BORDER, justifyContent: 'center' },
  colBreak: { width: '18%', padding: '6 5', borderRightWidth: THIN, borderColor: BORDER, justifyContent: 'center' },
  colHours: { width: '20%', padding: '6 5', justifyContent: 'center' },
  headerText: { fontWeight: 'bold' },
})

const ScheduleTable = ({ rows }: { rows: PledgeScheduleRow[] }) => {
  const padded: (PledgeScheduleRow | null)[] = [...rows, ...Array(Math.max(0, MAX_SCHEDULE_ROWS - rows.length)).fill(null)].slice(0, MAX_SCHEDULE_ROWS)
  return (
    <View>
      ...
      {padded.map((r, i) => (
        <View key={i} wrap={false} style={i < padded.length - 1 ? scheduleStyles.dataRow : scheduleStyles.dataRowLast}>
...
const pledgeWageStyles = StyleSheet.create({
  row: { flexDirection: 'row', borderBottomWidth: THIN, borderColor: BORDER, minHeight: 24 },
  rowLast: { flexDirection: 'row', minHeight: 24 },
  cellLabel: { width: '22%', padding: '5 4', borderRightWidth: THIN, borderColor: BORDER, justifyContent: 'center', fontWeight: 'bold' },
  cellValue: { width: '28%', padding: '5 4', borderRightWidth: THIN, borderColor: BORDER, justifyContent: 'center' },
  cellValueLast: { width: '28%', padding: '5 4', justifyContent: 'center' },
})
  ...
  return (
    <Document>
      <Page size="A4" style={sharedStyles.page}>
        <Text style={[sharedStyles.title, { marginBottom: 28 }]}>{PLEDGE_DOCUMENT_TITLE}</Text>
        <Text style={[sharedStyles.intro, { marginBottom: 12 }]}>{PLEDGE_INTRO_TEXT}</Text>

        <View style={sharedStyles.table}>
          <LabeledRow label="就業場所" minHeight={44}>
            <View style={[sharedStyles.freeText, { paddingVertical: 5 }]}>
          ...
          <LabeledRow label={'従事すべき\n業務内容'} minHeight={58}>
            <AutoFitFreeText text={p.businessContent} maxLines={4} widthPt={420} sizes={[9, 8.3, 7.6, 6.9, 6.2]} lineHeight={1.55} />
          </LabeledRow>
          ...
          <LabeledRow label={'賃金支払方法\n\n支払時の控除'} minHeight={58}>
            <View style={[sharedStyles.freeText, { paddingVertical: 5 }]}>
              ...
              <Text>賃金支払時の控除：源泉所得税</Text>
            </View>
          </LabeledRow>

          <LabeledRow label="交通費" last minHeight={46}>
            <View style={[sharedStyles.freeText, { paddingVertical: 5 }]}>
              <Text>{transportText}</Text>
              {transportSecondaryNote ? <Text>{transportSecondaryNote}</Text> : null}
            </View>
          </LabeledRow>
        </View>

        <Text style={[sharedStyles.footerText, { marginTop: 15, marginBottom: 11 }]}>{PLEDGE_CLOSING_TEXT}</Text>

        <View style={[sharedStyles.signatureRow, { marginTop: 5 }]}>
          <View style={sharedStyles.signatureCol}>
            <SealSideBySide showSeal={p.showSeal} sealSrc={COMPANY_SEAL_PATH} textColWidth={98} gap={-12}>
              ...
              <Text style={{ fontWeight: 'bold', marginTop: 4 }}>株式会社APパートナーズ</Text>
