...
const TRANSPORT_BODY_TEXT: Record<string, string> = {
  ...
  // 2026-07-29デモ指摘：アルバイト誓約書のみ「実費または定期代」を「実費精算」に文言変更（定額上限・原則定期代支給の文言を廃止）。
  // 雇用契約書（EmploymentContractPdf/EmploymentContractAndConditionsPdf）の'default'とは意図的に別キーにし、
  // 影響範囲を誓約書PDF（PledgePdf.tsx）のみに限定する。
  'pledge-default': '交通費実費支給　①最寄駅から勤務先までの最安経路とする。②交通費明細書及び定期ICカードの写し（エビデンス）が必要。ICカードは各自で用意。③エビデンスの提出確認が取れない交通費は、支払い対象外とする。',
...
// 2026-07-30変更：「契約更新上限の有無」欄は現状「無」固定表示のみのため、他の選択項目
// （getAgreementLaborText等）と同じ【　】括弧で囲む表現に統一し、選択されている値であることを
// 明示する（伊藤さん指摘：実物イメージでは丸・四角で囲まれているが、罫線で囲む部品は新規実装が
// 必要になるため、既存の【】表現に統一する方針で確認済み）。
...
// ===== アルバイト誓約書（pledges）専用の固定文言（2026-07-23追加。伊藤さん確認済み）=====
// レイアウト最終案（v5）を前提に、表題・冒頭文・末尾文を確定。誓約書という性質上、
// 雇用契約書（双方合意型「甲」「乙」表記）とは異なり、従業員から会社への一方的な誓約の体裁とする。
export const PLEDGE_DOCUMENT_TITLE = 'アルバイト誓約書'
export const PLEDGE_INTRO_TEXT =
  '私（従業員）は、株式会社ＡＰパートナーズ（以下「会社」という）に対し、下記の条件のもとで就業することを誓約いたします。'
// 末尾文言は雇用契約書と同一文言に統一（2026-07-23伊藤さん指示）。
export const PLEDGE_CLOSING_TEXT =
  '株式会社ＡＰパートナーズは本書にて提示した内容に相違ないことを保証し、従業員は上記提示内容を承諾する。'
