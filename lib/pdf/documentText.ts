...
const TRANSPORT_BODY_TEXT: Record<string, string> = {
  ...
  // なっていたため、gasパターンと同様に明示的な改行を入れ、読みやすい箇所で必ず改行されるようにした。
  'pass-gas': '定期代支給およびガソリン代支給【私有車通勤(最寄り駅まで) 12円 / km】　\n①定期代については最寄駅から勤務先までの最安経路での定期代とする。②支払上限は3万円/月とする。③エビデンスの提出確認が取れない交通費は支払い対象外とする。⑤私有車通勤については別途私有車通勤を許可する書面を提出し、規定を遵守すること。',
}
...
export const getFlexTimeText = (flexTime: string): string => {
  ...
  return '有　(毎月1日を起算日とし、1ヶ月を平均して1週あたり40時間を超えないものとする。)'
}
