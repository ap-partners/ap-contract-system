import { EmploymentContractPdf } from './verify-bundle.mjs';
import { renderToBuffer } from '@react-pdf/renderer';
import fs from 'fs';
const props = {
  contractType: '有期契約', documentLabel: '雇用契約書（有期）', employeeName: '猪野正明',
  employeeAddress: '東京都渋谷区神南一丁目2番3号 サンプルマンション405号室（かなり長い住所で2行になるかテスト用）',
  workLocationName: '本社', workLocationAddress: '東京都新宿区新宿2-16-20', workLocationTel: '',
  ...
  pattern: 'A', bonusType: 'あり', overtime: '有', showSeal: false,
};
const buf = await renderToBuffer(EmploymentContractPdf(props));
fs.writeFileSync('/tmp/verify.pdf', buf);
